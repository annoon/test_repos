...
if True:
    ...
    filenames = ['running.txt', 'walking.txt', 'standing.txt', 'standing_arm.txt']
    ...
    f = open('clfs/test' + str(WINDOW_SIZE) + '-' + str(STEP) + '.clf', 'w')
    pickle.dump(clf, f)

    labelsPredict = clf.predict(dataTest)

    score = clf.score(dataTest, labelsTest)
    cvscores = cross_validation.cross_val_score(clf, dataTest, labelsTest, cv=5)
    #for x in dataTest:
    #    print x
    #print ws,
    print ' ',
